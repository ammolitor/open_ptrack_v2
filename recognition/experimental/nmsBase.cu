/**
 * this file is extracted from src/openpose/net/nmsBase.cu
 */
#include <array>
#include <thrust/device_ptr.h>      // thrust is a parallel compute library
            ...
            const std::array<int, 4>& targetSize, const std::array<int, 4>& sourceSize, const Point<T>& offset)
{
    try
    {
        const auto num = sourceSize[0];             // in_n = 1
        const auto height = sourceSize[2];          // in_h = height
        const auto width = sourceSize[3];           // in_w = width
        const auto channels = targetSize[1];        // in_c = 57
        const auto maxPeaks = targetSize[2]-1;      // out_h - 1 = 128 - 1
        const auto imageOffset = height * width;    // in_h * in_w
        const auto offsetTarget = (maxPeaks+1)*targetSize[3];   // out_h * out_w = 128 * 3

        const dim3 threadsPerBlock2D{THREADS_PER_BLOCK_1D, THREADS_PER_BLOCK_1D};       // { 16u, 16u }
        const dim3 numBlocks2D{getNumberCudaBlocks(width, threadsPerBlock2D.x),         // 
                                getNumberCudaBlocks(height, threadsPerBlock2D.y)};
        const dim3 threadsPerBlock1D{THREADS_PER_BLOCK};                                // { 512u }
        ...
        // time = 1.24 ms
        const dim3 threadsPerBlockRegister{THREADS_PER_BLOCK_1D, THREADS_PER_BLOCK_1D, 1};      // { 16u 16u 1 }
        const dim3 numBlocksRegister{getNumberCudaBlocks(width, threadsPerBlockRegister.x),     // { width // 16, height // 16, n * c // 1 }
        ...
        const dim3 numBlocksWrite{getNumberCudaBlocks(imageOffset, threadsPerBlockWrite.x),
                                    getNumberCudaBlocks(num * channels, threadsPerBlockWrite.z)};       // WTF, should it be threadsPerBlockWrite.y ?
...
void nmsGpuFloatWrapper(
    ...
    nmsGpu<float>(dst, kernel, src, threshold, dstSize, srcSize, offset);
}