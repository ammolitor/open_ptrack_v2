...
int MTCNNTracking(MTCNN &detector, FR_MFN_Deploy &deploy) {
     ...
     * truth face we have set in advanced. The similarity score will be output at the cv::imshow windows.
     * -------
     * Args:
    ...
    for (int i = 1; i <= avg_face; i++) {
        faces = imread("/home/mkrzus/projects/faceswap/lib/faceswap/mark2/sorted" + format("/%5d.jpg", i));
...
//        sharpen(faces,faces);
    ...
    if (0) {
        cv::imshow("face average", face_avg);
    }
...
//        sharpen(frame,frame);
        ...
        for (int i = 0; i < faceInfo.size(); i++) {
            ...
            if (0) {
                imwrite("/mkrzus/github/open_ptrack_lite/Face-Recognition-Cpp/" + format("img/zzw_%5d.jpg", count), aligned);
                cv::imshow("crop face", aligned);
                cv::waitKey(0);
            }
...
int RetinaFaceTracking(RetinaFaceDeploy &deploy_track, FR_MFN_Deploy &deploy_rec) {
    ...
    vector<Mat> face_list;
    for (int i = 1; i <= 9; i++) {
        //faces = imread("/mkrzus/github/open_ptrack_lite/Face-Recognition-Cpp/" + format("img/zzw_%d_retina.jpg", i));
        // test random face from web as base face
        std::string file_path = "/home/mkrzus/projects/faceswap/lib/faceswap/mark2/sorted" + format("/0000%d.png", i);
        cout << file_path << endl;
        faces = imread(file_path);
//        GaussianBlur(faces,faces,Size( 3, 3 ), 0, 0);
//        sharpen(faces,faces);
        if (!faces.data){
            printf("sonuva\n");
    ...
    //    return -1;
    //}
    cv::VideoCapture cap("/home/mkrzus/Videos/burroughs.mp4.mp4");
...
//        sharpen(frame,frame);
        ...
        for (int i = 0; i < faceInfo.size(); i++) {
            ...
            // TODO: remember to set it to 1 before using this pipeline. This will get the ground truth image of your face for further calculating.
            if (0) {
                imwrite("/mkrzus/github/open_ptrack_lite/Face-Recognition-Cpp/" + format("img/zzw_%5d_retina.jpg", count),
                        aligned);
                cv::imshow("crop face", aligned);
                cv::waitKey(0);
            }
...
//            cerr << "inference cost: " << (double) (end - start) / CLOCKS_PER_SEC << endl;
        ...
        cv::imshow("image", result_cnn);
        cv::waitKey(1);
        std::string frame_name = format_frame_number("/home/mkrzus/github/open_ptrack_lite/Face-Recognition-Cpp/frames/", count);
        std::cout<< frame_name;
        cv::imwrite(frame_name, result_cnn);
