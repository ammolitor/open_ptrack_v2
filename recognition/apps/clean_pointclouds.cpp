...
class CleanerNode {
  ...
  public:
    ...
    CleanerNode(ros::NodeHandle& nh, std::string sensor_string, bool use_dynamic_reconfigure):
      node_(nh), it(node_)
      {
        ...
        catch(const std::exception& e)
        {
          ...
          ROS_WARN_STREAM("CleanerNode may not behave as expected!");
          // exit here
          //return 0;
        }
        ...
        //  ground_coeffs_sub = node_.subscribe(sensor_string + "/ground_coeffs/", 1, &CleanerNode::ground_coeffs_callback, this);
        //}
        ...
        point_cloud_approximate_sync_ = node_.subscribe(sensor_string + "/depth_registered/points", 1, &CleanerNode::callback, this);
        ...
        if (use_saved_ground_file){
          ...
          if (infile.good())
          {
            ...
            for (int row_ind = 0; row_ind < 4; row_ind++)
            {
              ...
              //ground_coeffs(static_cast<int>(row_ind)) = value;
     ...
     * \param[in] PersonClusers to be filled by this function
     */
    PointCloudPtr create_foreground(const PointCloudT::ConstPtr& cloud_){
      int min_points = 30;
      int max_points = 5000;
      PointCloudT::Ptr cloud(new PointCloudT);
      *cloud = *cloud_;
